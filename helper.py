...
def play_stored_video(conf, model):
    source_vid = st.sidebar.selectbox(
        "Choose a video...", list(settings.VIDEOS_DICT.keys()))

    is_display_tracker, tracker = display_tracker_options()

    if source_vid:
        video_path = settings.VIDEOS_DICT[source_vid]

        try:
            st.sidebar.info("Opening video file...")
            vid_cap = cv2.VideoCapture(video_path)

            if not vid_cap.isOpened():
                st.sidebar.error(
                    "Failed to open video file. Check if the path is correct.")
                return

            st.sidebar.success("Video file opened successfully!")
            st_frame = st.empty()
            while vid_cap.isOpened():
                success, image = vid_cap.read()
                if success:
